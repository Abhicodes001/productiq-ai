  ...
  return (
    <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 space-y-5 shadow-xl">
      ...
      <div className="flex items-center justify-between border-b border-slate-800 pb-4">
        ...
        <span className="px-2.5 py-1 rounded-full bg-red-500/10 text-red-400 border border-red-500/20 text-[11px] font-semibold flex items-center gap-1">
        ...
        <div className="flex flex-wrap gap-2">
          {sampleQuestions.map((q, idx) => (
            <button
              ...
              onClick={() => handleAsk(q)}
              disabled={loading}
              className="px-3 py-1.5 rounded-lg bg-slate-800/80 hover:bg-slate-800 text-slate-300 hover:text-red-300 text-xs font-medium border border-slate-700/50 transition-colors"
            >
              {q}
            </button>
          ))}
      ...
      >
        <input
          ...
          className="flex-1 px-4 py-2.5 rounded-xl bg-slate-950 border border-slate-800 text-slate-200 placeholder-slate-500 text-sm focus:outline-none focus:border-red-500 focus:ring-1 focus:ring-red-500 transition-all"
        />
        <button
          type="submit"
          disabled={loading || !question.trim()}
          className="px-5 py-2.5 rounded-xl bg-red-600 hover:bg-red-500 disabled:opacity-50 text-white text-xs font-bold transition-all flex items-center gap-2 shadow-lg shadow-red-600/20"
        >
          {loading ? (
                  ...
                  <div key={idx} className="p-3 rounded-lg bg-slate-900 border border-slate-800/80 space-y-1.5">
                    <div className="flex items-center justify-between text-xs text-slate-300">
                      <span className="font-semibold text-red-400 flex items-center gap-1">
                        {cite.source_type === 'pdf' ? <FileText className="w-3.5 h-3.5 text-rose-400" /> : <Globe className="w-3.5 h-3.5 text-red-400" />}
                        {cite.source_name}
                      </span>
                      {cite.page_number && (
